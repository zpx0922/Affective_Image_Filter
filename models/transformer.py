...
class Transformer(nn.Module):
    ...
    def __init__(self, d_model=512, nhead=8, num_encoder_layers=3,
        ...
        decoder_layer = TransformerEncoderLayer(d_model, nhead, dim_feedforward,
                                                dropout, activation, normalize_before)
        decoder_norm = nn.LayerNorm(d_model)
        self.decoder_cs = TransformerEncoder(decoder_layer, num_decoder_layers, decoder_norm)
    ...
    def forward(self, style, mask, content, pos_embed_c, pos_embed_s):
        ...
        content = content + self.type_embedding(torch.zeros_like(content[:,:,0].long()))
        style = style + self.type_embedding(torch.zeros_like(style[:,:,0].long()) + 1)

        content_style = torch.cat([content,style],dim = 0)
        content_style = self.decoder_cs(content_style, src_key_padding_mask=mask, pos=pos_embed_s)

        hs = content_style[:1024,:,:]
